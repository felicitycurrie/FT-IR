function [S,E] = xls2struct6(fn,sn)
...
% find and remove totaly blank columns

zz = all(cellfun('isempty',t2),1);
t2(:,find(zz)) = [];
zz = all(cellfun('isempty',t2),2);
t2(find(zz),:) = [];
t1(find(zz),:) = [];
...
for i = 1:length(h)
  ...
  temp = h{i}(find(~isstrprop(h{i},'alpha') & ~isstrprop(h{i},'digit')));
  
  
  for j = 1:length(temp)
    h{i} = strrep(h{i},temp(j),'_');
  end
