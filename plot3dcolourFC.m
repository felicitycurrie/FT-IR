function plot3dcolourFC(data,info,x,y,z,col)
...
h = figure;
plot3(data(:,x),data(:,y),data(:,z),'w.')

if isa(info,'cell')
    for i = 1:r 
        text(data(i,x),data(i,y),data(i,z),info{i},'color',bob(i,:));
...
elseif isa(info,'char')
    ...
    l = int2str(info);
    for i = 1:r 
        text(data(i,x),data(i,y),data(i,z),l(i,:),'color',bob(i,:));
    end
